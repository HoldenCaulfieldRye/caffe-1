...
void ThresholdLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  Dtype* bottom_data = bottom[1]->cpu_data()

  caffe_set(labels_->count(), 0, labels_) // is this neccesary? Why isn't caffe_set using cblas in general? (doesn't matter for 0 case)
  for (int i = 0; i < label_count; ++i) { // assumes labels are integers starting from 0
    labels_data[bottom_data[i]] += 1
  }
  ...
  for (int i = 0; i < num; ++i) {
    for (int j = 0; j < dim; ++j) {
      prior_data[i * dim + j] = labels_[j];
    }
  }

  caffe_cpu_scale(count, 1.0 / label_count, prior_data, prior_data)
...
Dtype ThresholdLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const Dtype* prior_data = diff_.cpu_data();
